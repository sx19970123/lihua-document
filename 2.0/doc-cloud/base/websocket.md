...
在 `com.lihua.websocket.config.WebSocketConfig` 中对 WebSocket进行了配置，指定了连接路径和拦截器等
...
在 `com.lihua.websocket.interceptor.WebSocketInterceptor` 中对连接握手进行拦截。因客户端发起连接时无法携带 token，防止恶意连接，客户端每次连接前会向后台请求一个一次性token，携带该token认证后才可放行连接
